...
Structure Entity_Main
  Timer_Send.l                    ; Timer f�r das Senden der Entities (Pos, L�schen, Erstellen)
  Timer_Check_Pos.l               ; Timer f�r das �berpr�fen der Position
EndStructure
...
Procedure Entity_Delete(ID)
  If Entity_Select_ID(ID)
    ...
    ; ############# Pointer zu dem Element l�schen
...
Procedure Entity_Position_Check(ID) ; Pr�ft, ob dieses Entity den Block betreten darf/ ob er t�tlich ist ... und Teleporter
  List_Store(*Pointer, Entity())
  If Entity_Select_ID(ID)
    Map_ID = Entity()\Map_ID
  ...
  If Entity_Select_ID(ID)
    If Entity()\Send_Pos <= Priority
      ...
      If Plugin_Event_Entity_Position_Set(Entity(), Map_ID, X.f, Y.f, Z.f, Rotation.f, Look.f, Priority.a, Send_Own_Client.a)
        ...
        If Entity()\Map_ID <> Map_ID ; ############## Wenn Kartenwechsel, dann: Neue ID_Client, Texte senden, Rang pr�fen
          If Map_Select_ID(Map_ID)
...
Procedure Entity_Send() ; Verwaltet das Bewegen, Erstellen und L�schen von Entities der Klienten
      ...
      ForEach Network_Client()\Player\Entities()
        ...
        If Entity_Select_ID(ID, 0)
          ...
          If Entity()\Map_ID <> Network_Client()\Player\Map_ID
            Delete = 1
          EndIf
          ; ######## Das Entitie von sich selbst l�schen
          If Network_Client()\Player\Entity
